function renderUserInfo(routeInfo, index){
    ...
    </p><p><strong>Duration:</strong> <span class="info-duration">${routeInfo[index].routeDuration}</span> <strong>Distance: </strong><span class="info-distance"> ${routeInfo[index].routeDistance}</span>    <span class="icon">${routeInfo[index].icon}</span></div>`) 
}
